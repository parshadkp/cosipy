...
def api():
    ...
    try:
        spec = importlib.util.spec_from_file_location('dc4_temporal_fit_test', directory/'dc4_temporal_fit.py')
...
def test_temporal_exposure_scaling_and_zero_overlap(api, setup, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(api, 'SpacecraftHistory', lambda *a, **k: SimpleNamespace(livetime=k['livetime']))
    monkeypatch.setattr(api, 'BinnedThreeMLPointSourceResponse', response)
